...
`the user request` is a journal name, ISSN, CN number, or sponsor name.

## Steps

### 1. Navigate

Use `chrome-devtools.navigate_page` → `https://navi.cnki.net/knavi`

### 2. Search + extract results (single evaluate_script, NO wait_for)

Use `chrome-devtools.evaluate_script`. Replace `QUERY_HERE` with actual search term:
...
```
期刊检索 "the user request"（共 {count} 条）：
