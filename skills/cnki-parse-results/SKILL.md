...
Use `chrome-devtools.take_snapshot`. Verify the page contains "条结果". If not, inform the user that no search results page is currently open.
...
If JavaScript returns empty (DOM structure changed), use `chrome-devtools.take_snapshot` and parse the accessibility tree manually:
