...
```bash
python "<path-to-this-skill>/scripts/push_to_zotero.py" /tmp/papers.json
```

Resolve `<path-to-this-skill>` to the installed `cnki-export` skill directory. In a standard Codex install this is typically `C:\Users\<user>\.codex\skills\cnki-export`.
...
Query collections:
```bash
python "<path-to-this-skill>/scripts/push_to_zotero.py" --list
```
